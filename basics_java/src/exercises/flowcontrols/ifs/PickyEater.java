...
public class PickyEater {
    ...
    public static void main(String[] args) {
        ...
        if (timesFried >= 2 && timesFried <= 4 && chocolatedCovered.toLowerCase(Locale.ROOT).equals("y")) {
            System.out.println("Oh, it's like a deep-fried Snickers. That'll be a hit!");
        }

        if (timesFried == 2 && cheeseCovered.toLowerCase(Locale.ROOT).equals("y")) {
            System.out.println("Mmm. Yeah, he'll each fried cheesy doodles.");
        }

        if (isBroccoli.toLowerCase(Locale.ROOT).equals("y") && butterPats > 6 && cheeseCovered.toLowerCase(Locale.ROOT).equals("y")) {
            System.out.println("As long as the green is hidden by cheddar, it'll happen!");
        } else if (isBroccoli.toLowerCase(Locale.ROOT).equals("y")) {
